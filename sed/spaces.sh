...
if [ -f "$FILE" ]; then 
   ...
   #print on the same line spaces/tabs as a red background at the begining of line 
    if [[ "$line" =~ $REGEX_START ]]; then
      MATCH=`echo "$BASH_REMATCH"| sed 's/\t/|__TAB__|/g'` 
      echo -e -n "\e[41m$MATCH\e[49m" >> temp.txt
    fi 
   #print on the same line part of line which is correct-which doesn't have spaces/tabs at the begining 
   #and at the end of line 
    echo -e -n "$line"| sed -e 's/^[[:blank:]]+//' -e 's/[[:blank:]]\+$//' >>temp.txt

   #print on the same line spaces/tabs as a red background at the end of the line 
